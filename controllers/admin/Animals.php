...
class Animals extends CI_Controller{
	 ...
	 *@param $start - Starting of animals table's index to get query
	 *
	 */
    function index($start=0){
		$limit = 10;
        $data['animals'] = $this->Animals_model->get_limit_animals($limit,$start);
		//pagination
		$config['base_url'] = site_url('admin/animals/index');
		$config['total_rows'] = $this->Animals_model->get_count_animals();
		$config['per_page'] = 10;
        ...
        $data['_view'] = 'admin/animals/index';
...
'types_id' => html_escape($this->input->post('types_id')),
...
'modified' => html_escape($this->input->post('modified')),
		...
		//update		
        if(isset($id) && $id>0){
			$data['animals'] = $this->Animals_model->get_animals($id);
            if(isset($_POST) && count($_POST) > 0){   
                $this->Animals_model->update_animals($id,$params);
                redirect('admin/animals/index');
            }else{
                $data['_view'] = 'admin/animals/form';
                $this->load->view('layouts/admin/body',$data);
            }
        } //save
		else{
			if(isset($_POST) && count($_POST) > 0){   
                $animals_id = $this->Animals_model->add_animals($params);
                redirect('admin/animals/index');
            }else{  
			    $data['animals'] = $this->Animals_model->get_animals(0);
                $data['_view'] = 'admin/animals/form';
                $this->load->view('layouts/admin/body',$data);
            }
		}
	...
	function details($id){
        $data['animals'] = $this->Animals_model->get_animals($id);
		$data['id'] = $id;
        $data['_view'] = 'admin/animals/details';
        $this->load->view('layouts/admin/body',$data);
	}
     ...
     */
    function remove($id){
        $animals = $this->Animals_model->get_animals($id);

        // check if the animals exists before trying to delete it
        if(isset($animals['id'])){
            $this->Animals_model->delete_animals($id);
            redirect('admin/animals/index');
        }
        else
            show_error('The animals you are trying to delete does not exist.');
    }
...
$this->db->or_like('types_id', $key, 'both');
...
$this->db->or_like('modified', $key, 'both');
        ...
        $data['animals'] = $this->db->get('animals')->result_array();
		...
		//pagination
		$config['base_url'] = site_url('admin/animals/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('users_id', $key, 'both');
$this->db->or_like('name', $key, 'both');
...
$this->db->or_like('modified', $key, 'both');
		...
		$config['total_rows'] = $this->db->from("animals")->count_all_results();
		$db_error = $this->db->error();
		if (!empty($db_error['code'])){
			echo 'Database error! Error Code [' . $db_error['code'] . '] Error: ' . $db_error['message'];
			exit;
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/admin/animals/print_template.php');
			$html = ob_get_clean();
